...
def cartLochigh(Q,personList,r, highestr):
    ...
    for subPersonList in comb:
        ...
        if (minX!=Q+1)and(minY!=Q+1):
            ...
            if r == highestr:
                ...
                return cartLochigh(Q,personList,r+1,highestr)
    del comb[:]
    return cartLochigh(Q,personList,r-1,r-1)

def printAnswer(persondict):
    for i in persondict.keys():
        x,y= cartLochigh(persondict[i][0],persondict[i][1],1,len(persondict[i][1]))
